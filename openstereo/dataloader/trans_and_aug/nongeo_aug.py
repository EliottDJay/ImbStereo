...
from utils.check import isNum, is_list, is_list_or_tuple, is_array



class SymandAsymMulAdjustments(object):
    def __init__(self, cfg, stage=None):
        """
        Generate two sets of factors, one of which affects the left and right imgs at the same time, the other
        generates different perturbations to the two views, and these two sets of operations are carried out successively
        :param p_op: conduct the operation
        """
        self.p_op = cfg.get("p_op", 1.)
        self.intervel = cfg.get("intervel", [0.8, 1.2])
        asym_inter = cfg.get("asym_inter", None)
    ...
    def _symandasymmul(self):
        sym_factor = np.random.uniform(self.intervel[0], self.intervel[1])
        asym_factor = np.random.uniform(self.asym_intervel[0], self.asym_intervel[1], 2)
        return sym_factor, asym_factor
    
    def _info(self):
        raise NotImplementedError


class RandomBrightnessContrastSymandAsym(SymandAsymMulAdjustments):
    """
        in HITNet:
        We first perturb the brightness and contrast of left and right images by using random symmetric and asymmetric
        multiplicative adjustments. Symmetric adjustments are sampled within [0.8, 1.2] interval and asymmetric between [0.95, 1.05].
        """
    def __call__(self, sample):
        if self._conduct():
            sample = ToPILImage()(sample)
            sym_factor, asym_factor = self._symandasymmul()
            sample['left'] = F.adjust_brightness(sample['left'], sym_factor)
            sample['right'] = F.adjust_brightness(sample['right'], sym_factor)
            sample['left'] = F.adjust_brightness(sample['left'], asym_factor[0])
            sample['right'] = F.adjust_brightness(sample['right'], asym_factor[1])
            sym_factor, asym_factor = self._symandasymmul()
            sample['left'] = F.adjust_contrast(sample['left'], sym_factor)
            sample['right'] = F.adjust_contrast(sample['right'], sym_factor)
            sample['left'] = F.adjust_contrast(sample['left'], asym_factor[0])
            sample['right'] = F.adjust_contrast(sample['right'], asym_factor[1])
            sample = ToNumpyArray()(sample)
        return sample
    
    def _info(self):
        Log.info(f"Using RandomBrightnessContrast symmetric and asymmetric multiplicative Transfomer at {self.stage} stage.")

    def epoch_pass(self, sample):
        if self._conduct(): 
            _, _ = self._symandasymmul()
            _, _ = self._symandasymmul()
        return sample
...
class SymorAsymAdjustments(object):
    ...
    def _symorasym(self):
        if np.random.random() < self.p_asym:
...
class RandomGamma(SymorAsymAdjustments):
    # np.random.uniform(0.7, 1.5)  # adopted from FlowNet AANet
    def __call__(self, sample):
        if self._conduct():
...
class ChromaticAugmentation(object):
    def __init__(self, chromatic_cfg):
        self.p_op = chromatic_cfg.get('prob', 1.0)
        self.chromatic_strategy = chromatic_cfg.get('chromatic_strategy', 'fixed')  # fixed, random, curriculum
        self.sample_aug_init = chromatic_cfg.get('sample_aug_init', 1)  # this could be changed during curriculum process
        ...
        self._paras_init()
        self._log_info()
        self.sample_aug = copy.deepcopy(self.sample_aug_init)  # it is ok to deep copy
    ...
    def _paras_init(self):
        ...
        if self.chromatic_strategy in ['random', 'curriculum']:
            self.shuffle = True
            ...
            self.considered_aug_init = np.arange(num)
            # when use fixed strategy, the number should align with the aug operations number
        else:
            self.shuffle = False
    ...
    def _log_info(self):
        ...
        elif self.chromatic_strategy == 'curriculum':  #TODO delete
            pass
        else:
